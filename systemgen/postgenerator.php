...
function showAllPost($type){
    ...
    if($type==1){
        ...
        while($post=mysqli_fetch_array($result)){
            $postId=$post['id'];
            echo "<div class='card col-md-4 m-4' style='width: 20.5rem;'>";
...
function categoryPost($categoryId,$type){
    $db=dbConnect();
    if($type==1){
        $qry = "SELECT * FROM post WHERE posttype=$type AND category=$categoryId";
    }else{
